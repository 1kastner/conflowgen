...
class TestClippedLogNormal(unittest.TestCase):
    ...
    def test_lognorm_properties(self):
        self.assertAlmostEqual(self.cln._lognorm.mean(), 5 + 1)  # pylint: disable=protected-access
        self.assertAlmostEqual(self.cln._lognorm.var(), 2)  # pylint: disable=protected-access

    def test_reversed(self):
        reversed_cln = self.cln.reversed()
        self.assertAlmostEqual(reversed_cln._lognorm.mean(), 5 + 1)  # pylint: disable=protected-access
